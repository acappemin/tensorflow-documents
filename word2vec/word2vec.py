...
# using each target word to predict their context words
...
num_steps = data_len // batch_size * 10

with tf.Session(graph=graph) as session:
	# We must initialize all variables before we use them.
	init.run()
	print('Initialized %d', num_steps)
...
def save_variables(variables):
	with open('variables.py', 'wb') as f:
		for variable in variables:
			cPickle.dump(variable, f)


save_variables([dictionary, reverse_dictionary, final_embeddings])
